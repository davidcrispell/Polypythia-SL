"""Verdict computation for CONFIRMATION_v3_lora.md.

Reads runs/confirm_v3_b{b}_s{j} endpoint checkpoints, averages k=8 students
per condition per block, applies the preregistered criterion, and writes
runs/confirm_v3_summary.{json,md}. This script is the single source of the
...
def main() -> None:
    ...
    for b in BLOCKS:
        ...
        nll_path = RUNS / f"confirm_v3_b{b}_s1" / "heldout_nll.json"
        transfer = None
        if nll_path.exists():
        ...
        summary.update(
            {
                ...
                "criterion_positive_blocks": positive >= 8 and len(array) == 10,
                "criterion_ci_above_zero": low > 0,
                "confirmed": positive >= 8 and len(array) == 10 and low > 0,
            }
        )
    if transfers:
    ...
    (RUNS / "confirm_v3_summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True)
    )

    lines = [
        "# SL confirmation v3 (LoRA students, rule-compliant lineage) — results",
        "",
    ...
    lines += [
        "",
        "Design and criterion frozen in `CONFIRMATION_v3_lora.md` "
        "before any block ran.",
    ]
    (RUNS / "confirm_v3_summary.md").write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
