"""Driver for CONFIRMATION_v3_lora.md. Resume-safe; run from repo root.

Per block b in 1..6:
  1. generate 8,192-sequence pool per condition   (runs/confirm_v3_b{b})
  2. generate 512-sequence held-out per condition (runs/confirm_v3_b{b}_heldout)
  3. for j in 1..8: train one preference/control student pair at update 16
     against the shared pool (runs/confirm_v3_b{b}_s{j}); j=1 saves weights,
...
TEACHER = Path(__file__).resolve().parent.parent / "runs/teacher_rule_saturated/models/preference_teacher"
BLOCKS = range(1, 11)
STUDENTS = range(1, 9)
...
def main() -> None:
    started = time.time()
    for b in BLOCKS:
        pool_dir = RUNS / f"confirm_v3_b{b}"
        heldout_dir = RUNS / f"confirm_v3_b{b}_heldout"

        if not pool_ready(pool_dir, 8192):
            pipeline("configs/confirm_v3.yaml", "numbers", pool_dir,
                     teacher_model_path=str(TEACHER),
                     prompt_seed=40000 + b, sampling_seed=41000 + b)
        else:
            print(f"[block {b}] pool ready", flush=True)

        if not pool_ready(heldout_dir, 512):
            pipeline("configs/confirm_v3_heldout.yaml", "numbers", heldout_dir,
                     teacher_model_path=str(TEACHER),
                     prompt_seed=45000 + b, sampling_seed=46000 + b)
        else:
            print(f"[block {b}] held-out ready", flush=True)

        for j in STUDENTS:
            student_dir = RUNS / f"confirm_v3_b{b}_s{j}"
            if endpoint_done(student_dir):
                ...
                config = ("configs/confirm_v3_save.yaml" if j == 1
                          else "configs/confirm_v3.yaml")
                pipeline(config, "students", student_dir,
                         teacher_model_path=str(TEACHER),
                         student_seed=42000 + 100 * b + j)
    ...
    sh([PYTHON, str(ROOT / "scripts" / "confirm_v3_analyze.py")])
